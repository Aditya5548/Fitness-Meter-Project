...
function Workdaily(){
    ...
    const Viewall=async ()=>{
        //push-up
        const pushupdata=await fetch(`http://localhost:4000/Wbydate/${localStorage.getItem('user')}/Push-Up`);
        const result1= await pushupdata.json();

        //pull-up
        const pullup=await fetch(`http://localhost:4000/Wbydate/${localStorage.getItem('user')}/Pull-Up`);
        const result2= await pullup.json();

        //pull-up
        const Hamercut=await fetch(`http://localhost:4000/Wbydate/${localStorage.getItem('user')}/Hamercut`);
        const result3= await Hamercut.json();

        //situp
        const situp=await fetch(`http://localhost:4000/Wbydate/${localStorage.getItem('user')}/Situp`);
        const result4= await situp.json();
        
        //Bench Press
        const Benchpress=await fetch(`http://localhost:4000/Wbydate/${localStorage.getItem('user')}/Bench-press`);
