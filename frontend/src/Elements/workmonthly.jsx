...
function workmonthly(){
    ...
    const Viewall=async ()=>{
        //push-up
        const pushupdata=await fetch(`${import.meta.env.VITE_URL}/Wmonth/${localStorage.getItem('user')}/Push-Up`);
        const result1= await pushupdata.json();

        //pull-up
        const pullup=await fetch(`${import.meta.env.VITE_URL}/Wmonth/${localStorage.getItem('user')}/Pull-Up`);
        const result2= await pullup.json();

        //pull-up
        const Hamercut=await fetch(`${import.meta.env.VITE_URL}/Wmonth/${localStorage.getItem('user')}/Hamercut`);
        const result3= await Hamercut.json();

        //situp
        const situp=await fetch(`${import.meta.env.VITE_URL}/Wmonth/${localStorage.getItem('user')}/Situp`);
        const result4= await situp.json();
        
        //Bench Press
        const Benchpress=await fetch(`${import.meta.env.VITE_URL}/Wmonth/${localStorage.getItem('user')}/Bench-press`);
    ...
    return (
     <>
            <h2  className='text-center'>Your 30 days Workout</h2>
